		...
		switch(command) {
			...
			case prefix+'speed':
			    let timestamp = speed();
					...
					case prefix+'sewa':
  var donasibut = [
			{ urlButton: { displayText: `Info Lebih Lanjut`, url : `${setting.web}` } },
		]
var teks = `──「 SEWA BOT 」──

Hi ${pushname} 👋🏻 UNTUK SEWA BOTZ AKAN DI KENAI
BIAYA 5K DALAM JANGKA WAKTU PERMANENT
\`\`\`GOPAY : SCAN QRIS DI ATAS\`\`\`
\`\`\`OVO : SCAN QRIS DI ATAS\`\`\`
\`\`\`DANA : SCAN QRIS DI ATAS\`\`\`
\`\`\`PULSA : ${setting.donasiPulsa}\`\`\`

Terimakasih untuk kamu yang Sudah Sewa_*`
 conn.sendMessage(from, { caption: teks, image: {url: `${setting.donasiQris}`}, templateButtons: donasibut, footer: 'SEWA DAN DONASI UNTUK PERKEMBANGAN BOT MD', mentions: [sender]} )
			    break
			case prefix+'donasi':
            case prefix+'donate':
  var donasibut = [
